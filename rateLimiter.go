...
import (
	...
	"github.com/meshhq/meshCore/Godeps/_workspace/src/github.com/hjr265/redsync.go/redsync"
	"github.com/meshhq/meshCore/lib/meshLog"
	"github.com/meshhq/meshCore/lib/meshRedis"
)

// Default values for the GoHTTP RateLimiter.
const (
...
// RateLimitInfo models the initilization parameters for a RateLimiter.
...
type RateLimiter struct {
	...
	// retries represents the max amount of retires to begin the window
	retries int

	// delay is the time to wait between retries to create or enter a new window
	delay int64
...
// NewRateLimiter is a factory method for creating a RateLimiter
func NewRateLimiter(limitInfo *RateLimitInfo) *RateLimiter {
...
func (r *RateLimiter) Enter() error {
	...
	redisSession := meshRedis.NewSession()
...
func (r *RateLimiter) redMutexForTask(factor float64, delay int64) *redsync.Mutex {
	// Grab the pool
	redisPool := meshRedis.UnderlyingPool()
