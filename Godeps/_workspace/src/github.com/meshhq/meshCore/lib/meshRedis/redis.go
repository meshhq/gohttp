...
// AtomicPushOnListWithExpiration pushes a token, and key to a list with
// expiration of the list in terms of Milliseconds
func (s *RedisSession) AtomicPushOnListWithExpiration(key string, value string, msToExpire int64) error {
	err := s.connection.Send("MULTI")
	err = s.connection.Send("RPUSH", key, value)
	err = s.connection.Send("PEXPIRE", key, msToExpire)
	_, err = s.connection.Do("EXEC")
	return err
}
